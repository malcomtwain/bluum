...
export function drawHookText(
  ...
  let y = style.position === 'top' ? canvasHeight * 0.15 : 
          style.position === 'middle' ? canvasHeight * 0.5 : 
          canvasHeight * 0.85;
  ...
  // Calculate font size (text-2xl equivalent)
  const fontSize = Math.floor(canvasWidth * 0.06);

  // Set font
  ctx.font = `${fontSize}px "TikTok Display Medium", "Apple Color Emoji"`;
