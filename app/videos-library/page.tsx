...
import {
  ...
  import { uploadFileWithFallback, getFileUrl } from '@/lib/supabase';
                ...
                return (
                  <div key={clip.id} className={`group relative rounded-xl overflow-hidden bg-gradient-to-b from-black/40 to-black/20 dark:from-white/20 dark:to-white/10 border border-white/10 hover:border-white/20 transition transform hover:-translate-y-0.5 hover:shadow-lg` }>
                    <div className="relative aspect-[9/16]">
                      <video src={clip.path} className="w-full h-full object-cover" muted autoPlay loop playsInline />
                      <button onClick={()=>toggleSelectClip(clip.id)} className={`absolute top-2 left-2 w-5 h-5 rounded border ${selected ? 'bg-[#5564ff] border-[#5564ff]' : 'bg-black/40 border-white/60'} transition`}></button>
